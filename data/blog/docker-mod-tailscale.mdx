...
tags:
  - docker
  - philosophy
summary: "We created a universal Docker mod for Tailscale, which lets you slap Tailscale on the side of existing Docker containers from linuxserver.io. In this article, Xe explains how you can use this and why it's funny."
images: ['/images/hero/shipyard-vibes.png']
---
...
<HeroImage
  src="/images/hero/shipyard-vibes.png"
  desc="flat color, shipyard, containers, mountains, no humans, space needle"
  generator="Ligne Claire v1.5"
  width={1280}
  height={512}
/>
...
<ConvSnippet authors={props.authorDetails} name="Xe Iaso">
  You can also base your Docker images on the `lsiobase/alpine:3.17` image, which is a minimal
  [Alpine Linux](https://alpinelinux.org) with Docker mod support. This can be used to adapt your
  existing containers into nodes on your tailnet. You can also use [Ubuntu](https://ubuntu.com) with
  `lsiobase/ubuntu:jammy` as the base image. The cloud's the limit!
</ConvSnippet>
...
services:
  code-server:
    ...
    environment:
      ...
      - DOCKER_MODS=ghcr.io/tailscale-dev/docker-mod:main|linuxserver/mods:code-server-nodejs|linuxserver/mods:code-server-npmglobal
      # tailscale information
      - TAILSCALE_STATE_DIR=/var/lib/tailscale
