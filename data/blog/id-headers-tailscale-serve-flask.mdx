...
If you've got an application you want to share with the whole internet, chances are you'll have to implement some kind of login system. But login systems are fraught with peril, and are one of the most complicated things that we programmers have to implement over and over and over again. It's especially frustrating when you don't actually need to expose the application to the whole internet; say, a service you only want to share with friends or co-workers.
...
Funnel and Serve work together to serve public traffic to the internet and private traffic to your tailnet. But for traffic served over Tailscale Serve, you've got all the parts you need to skip the hard work of login.

We're connected over Tailscale. In order to authenticate with Tailscale, you have to pass a two-factor auth check, and your identity provider tells Tailscale who you are. This means every connection comes with a username, so you can _completely bypass_ the need to find new and interesting ways to tell your application who the client is. Tailscale already knows!

Since version 1.44, the Tailscale Serve process now fills this information into headers while proxying requests back to your service. Today we're going to be talking about how you can use Tailscale as your authentication system for internal services with the magic of HTTP headers.
...
If you turn Tailscale off, or access from another device, you should see the original “logged out” page. DNS caches may make this finicky at times. If you are uncertain if this is the case for you, either restart your browser or open another browser.
