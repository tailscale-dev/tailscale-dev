---
title: How to use Tailscale without using Tailscale
date: 2023-04-01
authors: ['xe']
tags:
  - funnel
...
As a philosopher, I find it useful to keep up on the latest trends in technology. One of the premiere websites with which I used to do this is a site known as [Hacker News](https://news.ycombinator.com). My friends and coworkers worry about how I use this website, because it takes on it can be...special, and I tend to view it as a bit of a surrealist comedy. However, my dear philosopher friend xeonmc asked a question that that served as a fount of inspiration. He asked:

<BlockQuote>Is it possible to use this to host a Headscale server from behind NAT?</BlockQuote>

<XesiteConv name="Aoi" mood="wut">
  Wait, what? Is that person asking you how the use Tailscale in a way that makes you avoid using
  Tailscale? That's like asking how to use a car without using a car.
</XesiteConv>

Oh yes [my dear fox](https://xeiaso.net/characters#aoi), it is. And today I am going to show you how you would accomplish such an accursed. Buckle up, because this is going to be a wild ride.
...
[Funnel](https://tailscale.com/kb/1223/tailscale-funnel/) is a feature of Tailscale that allows you to expose a service on your network to the internet. This is the missing part of this equation, and what will allow us to use Tailscale (Funnel) without using Tailscale (the SaaS control plane) for the rest of the network.
...
- Somewhere to run virtual machines (I use [waifud](https://github.com/Xe/waifud)).
...
<Warning>
  ...
  known issue with that image, cloud-init, and NixOS conflicting on how user creation works.
</Warning>
...
<XesiteConv name="Mara" mood="hacker">
  This isn't required, but it's fun to demonstrate that things will come back up when you reboot the
  machine.
</XesiteConv>
...
Then wait a minute or two for the DNS gods to smile upon your face and open the URL in your favoirte browser (such as [GNOME Web](https://wiki.gnome.org/Apps/Web/)). It should return a 404 page. This is expected. Now let's create a namespace for our nodes:

```
headscale namespaces create casa
```

Now spin up another Linux VM in waifud such as an [Amazon Linux 2](https://aws.amazon.com/amazon-linux-2/):
...
100.64.0.1 geordi-coral-bits casa linux offline
```
